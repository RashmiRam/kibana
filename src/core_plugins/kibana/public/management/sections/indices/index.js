...
uiRoutes
.defaults(/management\/haystack\/indices/, {
  resolve: indexPatternsResolutions
});

uiRoutes
.defaults(/management\/haystack\/index/, {
  resolve: indexPatternsResolutions
});
    ...
    link: function ($scope) {
      ...
      $scope.$watch('defaultIndex', function () {
        ...
        $scope.indexPatternList = ids.map(function (id) {
          return {
            id: id,
            url: kbnUrl.eval('#/management/haystack/indices/{{id}}', {id: id}),
...
management.getSection('kibana').register('indices', {
  ...
  order: 0,
  url: '#/management/haystack/indices/'
});
